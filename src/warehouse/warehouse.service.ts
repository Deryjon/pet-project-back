...
export class WarehouseService {
  ...
  private mapItem(i: any, blind = false) {
  ...
  async getInventorySession(id: string, requestContext: CompanyRequestContext) {
    ...
    return { ...s, blind, items: s.items.map((i: any) => this.mapItem(i, blind)) };
  }
  ...
  async approveInventory(id: string, requestContext: CompanyRequestContext) {
    const c = this.ctx(requestContext); this.manager(c);
    return runSerializableTransaction(this.db, async (tx) => {
      const s: any = await tx.inventorySession.findFirst({ where: { id, ...this.scope(c) }, include: { items: true, shop: true } }); if (!s) throw new NotFoundException('Session not found'); if (s.status === 'COMPLETED') return { success: true, id, status: 'COMPLETED', idempotent: true }; if (s.status !== 'REVIEW') throw new BadRequestException('Inventory is not ready for approval'); const applicableItems = await this.canonicalInventoryItems(tx, s.items); if (!applicableItems.length) throw new BadRequestException('Inventory has no positions'); if (applicableItems.some((i: any) => ['NOT_COUNTED', 'RECOUNT_REQUIRED', 'SHORTAGE', 'SURPLUS'].includes(i.status) || i.countedQuantity === null)) throw new BadRequestException('All discrepancies must be reviewed before approval');
